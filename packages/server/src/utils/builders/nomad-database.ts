...
// The DB's data dir lives in a docker NAMED volume named after the app.
const dataVolumeName = (db: NomadDatabaseInput): string => `${db.appName}-data`;

// A docker `mount { type = "volume" }` stanza — a REAL persistent docker named
// volume (unlike the `volumes = ["name:/path"]` list, where Nomad turns a bare
// name into an alloc-dir relative bind that is wiped on every reschedule). This
// is the exact primitive the app/compose builder uses (see namedVolumeMounts in
// nomad.ts), so managed DBs now persist the same way apps do.
const volumeMountStanza = (source: string, target: string): string =>
	`        mount {
          type   = "volume"
          target = ${hclString(target)}
 ...
 * - the data dir on a docker NAMED volume (`<appName>-data`) via a
 *   `mount { type = "volume" }` stanza — persists across restarts AND
 *   reschedules (see volumeMountStanza), plus any user-defined mounts.
...
export const generateDatabaseNomadJob = (db: NomadDatabaseInput): string => {
	...
	// Host binds stay in the docker `volumes` list (an absolute host path is a
	// genuine bind mount). Named volumes — the data dir plus any user-configured
	// named volume — go through `mount { type = "volume" }` stanzas so they are
	// REAL persistent docker named volumes (see volumeMountStanza). The old code
	// put the data dir in the `volumes` list as "<appName>-data:/path", which
	// Nomad silently turned into an alloc-dir bind that vanished on reschedule.
	const bindVolumes: string[] = [];
	const mountBlocks: string[] = [];
	for (const m of db.mounts || []) {
		if (m.type === "volume" && m.volumeName)
			mountBlocks.push(volumeMountStanza(m.volumeName, m.mountPath));
		else if (m.type === "bind" && m.hostPath)
			bindVolumes.push(`${m.hostPath}:${m.mountPath}`);
	}
	// Add the managed data-dir named volume unless the user already mounted their
	// own storage at the data path.
	if (!(db.mounts || []).some((m) => m.mountPath === db.dataPath))
		mountBlocks.unshift(volumeMountStanza(dataVolumeName(db), db.dataPath));
	const volumesLine = bindVolumes.length
		? `\n        volumes = [${bindVolumes.map(hclString).join(", ")}]`
		: "";
	const mountsBlock = mountBlocks.length ? `\n${mountBlocks.join("\n")}` : "";
...
${ports.join("\n")}
    ...
    task "db" {
      ...
      config {
        ...
        ports   = ["db"${db.externalPort ? ', "external"' : ""}${(db.extraPorts || []).map((p) => `, "p${p}"`).join("")}]${volumesLine}${mountsBlock}${commandLine}${argsLine}
      }
...
export const getBuildNomadDatabaseCommand = (
	...
	// The data dir is a docker named volume declared via a `mount { type =
	// "volume" }` stanza (see generateDatabaseNomadJob); docker creates it on
	// first run and reuses it on every redeploy/reschedule, so no pre-provisioning
	// step is needed.
