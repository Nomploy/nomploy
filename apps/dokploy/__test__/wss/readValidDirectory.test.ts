...
vi.mock("@nomploy/server/constants", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("@nomploy/server/constants")>();
	return {
		...actual,
		paths: () => ({
			...actual.paths(),
...
const { readValidDirectory } = await import("@nomploy/server");
