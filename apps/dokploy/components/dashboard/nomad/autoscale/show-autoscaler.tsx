...
const GroupCard = ({
	...
	// Existing groups start collapsed to a summary (the settings form is large);
	// a new group card opens expanded so you can fill it in.
				...
				<div className="space-y-1.5">
					...
					{expanded ? (
						<CardDescription>
							...
							template + scaling policy. Target it from a job's node pool to
							drive its scaling.
						</CardDescription>
					) : (
						// Collapsed summary — the at-a-glance state without the big form.
						<div className="flex flex-wrap items-center gap-1.5 pt-0.5 text-xs">
							<Badge variant={form.enabled ? "default" : "outline"}>
								{form.enabled ? "enabled" : "disabled"}
							</Badge>
							<Badge variant="secondary">
								desired {group?.desiredNodes ?? form.minNodes} · running{" "}
								{d?.workerCount ?? "—"}
							</Badge>
							<Badge variant="outline">
								min {form.minNodes} / max {form.maxNodes}
							</Badge>
							{d && d.action !== "none" && (
								<Badge title={d.reason}>next: {d.action}</Badge>
							)}
						</div>
					)}
				</div>
			...
			<CardContent className="space-y-4">
				{expanded && (
					<>
						...
						<div className="space-y-2">
							<Label>
								Server types (tried in order until one is available)
							</Label>
							<Input
						...
						<p className="text-xs text-muted-foreground">
							Min/Max bound the <strong>total</strong> workers in this pool —
							both autoscaler-provisioned and manually added. Manually-added
							nodes are pinned: they count toward the minimum but the autoscaler
							never removes them; it only adds/removes its own nodes to keep the
							total in range.
						</p>
						...
						<div className="space-y-2">
							...
							<p className="text-muted-foreground text-xs">
								...
								capacity). Scales up if <b>either</b> exceeds its up-threshold,
								down only when <b>both</b> are below their down-thresholds.
							</p>
